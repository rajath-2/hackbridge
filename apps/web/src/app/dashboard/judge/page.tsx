...
export default function JudgeDashboard() {
  ...
  const notifications = useNotifications(event?.id, user?.id, "judge")
  ...
  if (loading) return <div className="min-h-screen dashboard-root flex items-center justify-center text-[var(--hb-muted)]">Loading...</div>
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between bg-[var(--hb-surface2)] border border-[var(--hb-border)] rounded-[8px] p-3 px-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[13px] font-medium text-[var(--hb-text)]">{event?.name || "Initializing..."}</span>
          </div>
          <div className="text-[12px] font-mono text-[var(--hb-cyan)]">Live Scoring Active</div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="flex flex-col gap-1.5">
              {teams.map(team => (
                ...
                >
                  ...
                  {team.risk_level === "High" && <Badge variant="amber">Flagged</Badge>}
                </div>
              ))}
              {teams.length === 0 && <div className="text-[11px] text-[var(--hb-muted)] italic p-4 text-center">No teams found for this event.</div>}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-2">
            <div className="text-[10px] text-[var(--hb-dim)] uppercase tracking-[0.08em] mb-1">
              Scoring Panel - {selectedTeam?.name || "No Team Selected"}
            </div>
            ...
            {selectedTeam ? (
              <Card variant="base" className="flex flex-col gap-6">
                ...
                <div>
                  {Object.entries(aiAnalysis?.scores || { "Code Quality": 0, "Complexity": 0, "Completion": 0 }).map(([crit, val]: [string, any]) => (
                    <ScoreBar key={crit} criterion={crit} score={val} />
                  ))}
                </div>

                <Card variant="ai">
                  {aiAnalysis?.evaluation || "Analyzing team repository and activity..."}
                </Card>
                ...
                <div className="pt-4 border-t border-[var(--hb-border)]">
                  ...
                  >
                    {submitting ? "Submitting..." : "Submit Round 1 Scores"}
                  </Button>
                </div>
              </Card>
            ) : (
