...
import { Grid2X2, MessageSquare, Radio, ShieldAlert } from "lucide-react"
...
export default function OrganizerDashboard() {
  ...
  const loadEventData = async (activeEvent: any) => {
    ...
    try {
      const [flagsData, teamsData, mentorsData] = await Promise.all([
        api.get(`/integrity/flags/event/${activeEvent.id}`),
        api.get(`/teams/event/${activeEvent.id}`),
        api.get(`/events/${activeEvent.id}/participants?role=mentor`)
      ]);
      setFlags(flagsData);
      setTeams(teamsData);
  ...
  const tickerContent = [
    ...
    { type: "SIGNAL", id: "◎", team: "DataNinjas", msg: "Mentor requested: Auth help" },
  ];
        ...
        <div className="ticker-track whitespace-nowrap flex items-center">
          {[...tickerContent, ...tickerContent, ...tickerContent].map((item, i) => (
            <span key={i} className="font-ui text-[10px] text-[var(--signal-live)] uppercase tracking-[0.1em] mx-4 flex items-center gap-2">
              <span>{item.type === 'COMMIT' ? '●' : (item.type === 'INTEGRITY' ? '⚑' : '◎')}</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[240px] flex-shrink-0 bg-[var(--surface-1)] border-r border-[var(--border)] flex flex-col sticky top-[32px] h-[calc(100vh-32px)]">
          ...
          <button 
            ...
            className={`px-6 py-2 flex items-center gap-3 h-[40px] transition-all border-l-[3px] font-ui text-[12px] uppercase tracking-wider ${activeTab === 'integrity' ? "text-[var(--text-primary)] border-[var(--signal-live)] bg-[var(--signal-live)]/5" : "text-[var(--text-secondary)] border-transparent hover:text-[var(--text-primary)] hover:bg-white/5"}`}
          >
            <ShieldAlert size={14} />
            Integrity
            {activeFlagsCount > 0 && (
          ...
          <div className="py-5 px-6 t-section mt-4">Support</div>
          <div className="px-6 py-2 flex items-center gap-3 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 font-ui text-[12px] h-[40px] cursor-pointer border-l-[3px] border-transparent transition-all uppercase tracking-wider">
            <MessageSquare size={14} />
            Mentor Pings
            {mentorPingsCount > 0 && (
              <span className="ml-auto font-ui text-[10px] px-1.5 py-0.5 rounded-[3px] bg-[var(--signal-ping)]/15 text-[var(--signal-ping)]">{mentorPingsCount}</span>
            )}
          </div>
        ...
        <main className="flex-1 min-w-[720px] flex flex-col h-full overflow-y-auto">
          ...
          <div className="p-[32px_40px] flex flex-col flex-1">
            ...
            <div className="mb-[32px]">
              ...
              <div className="font-body text-[13px] text-[var(--text-secondary)] flex items-center gap-2">
                <span>{event?.name || "Hackathon"}</span>
                <span className="opacity-30">·</span>
                <span>{teams.length} Nodes Online</span>
                <span className="opacity-30">·</span>
                <span className="text-[var(--signal-live)]">Uplink Nominal</span>
              </div>
            ...
            <div className="grid grid-cols-4 gap-[16px] mb-[24px]">
              ...
              <StatCard label="Mentor Pings" value={mentorPingsCount.toString().padStart(2, '0')} sub="Awaiting technician ACK" variant="ping" />
              <StatCard label="Commits Today" value="84" sub="Last detected 2m ago" variant="info" />
            ...
            <div className="grid grid-cols-12 gap-[24px] mb-[24px]">
              {/* Broadcast Composer */}
              <div className="col-span-12 flex flex-col gap-3 bg-[var(--surface-1)] border border-[var(--border)] rounded-[4px] p-5">
                <div className="t-section text-[var(--text-secondary)] uppercase">
                  Global Comm Channel
                </div>
                <div className="flex gap-3">
                  <Input 
                    ...
                    className="flex-1 font-body text-[13px] bg-[var(--surface-2)] border-[var(--border)] h-[40px] text-[var(--text-primary)] focus:border-[var(--signal-live)] transition-all"
                  />
                  <Button onClick={handleBroadcast} className="px-8 h-[40px] bg-[var(--signal-live)] text-[var(--void)] font-bold hover:bg-[var(--cta-hover)]">SEND_SIGNAL</Button>
                </div>
              </div>

              {/* Main Content Sections based on Tab */}
              {activeTab === "overview" ? (
          ...
          <div className="h-[32px] mt-auto flex-shrink-0 bg-[var(--surface-2)] border-t border-[var(--border)] flex items-center">
            <div className="flex-1 border-r border-[var(--border)] flex items-center justify-center t-micro uppercase">
              <span className="text-[var(--signal-live)] mr-2">●</span> BROADCAST_NOMINAL
            </div>
            <div className="flex-1 border-r border-[var(--border)] flex items-center justify-center t-micro uppercase">
              <span className="text-[var(--signal-ping)] mr-2">●</span> {mentorPingsCount} PINGS_ACTIVE
            </div>
            <div className="flex-1 flex items-center justify-center t-micro uppercase">
              <span className="text-[var(--signal-info)] mr-2">●</span> INTEGRITY_WATCH_STABLE
            </div>
          </div>
        </main>
      </div>
